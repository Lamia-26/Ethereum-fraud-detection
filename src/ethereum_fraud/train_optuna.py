...
Lancement :
    python -m ethereum_fraud.train_optuna
...
def objective(trial, spec: ModelSpec, x_train, y_train, cv: int) -> float:
    """Fonction objectif Optuna : score moyen de validation croisee (ROC AUC)."""
...
def describe_registered_version(
    ...
    description = (
        f"Modele : {result.spec.name} | "
        f"Sampler : TPE | "
        f"n_trials={n_trials} | cv={cv} | "
        f"Hyperparametres : {result.study.best_params} | "
